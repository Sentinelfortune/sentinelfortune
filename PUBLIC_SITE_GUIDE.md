...
Product covers are **not** managed here. They come from the Shop Worker's catalogue response
as a `coverImageUrl`, and the public site never holds a product image. Where no cover is
returned, the card falls back to a brand plate rather than a broken image.
...
The Worker suite (`cd shop-worker && npm test`) covers the backend, which this release does
not touch. Run it if you change the Worker; it is not part of publishing the site.
...
## 7. The storefront is not connected yet

`shop/shop-config.js` still holds `REPLACE_WITH_SHOP_WORKER_URL`. That is deliberate for this
release: pointing the public production site at a test Worker is a decision for the Owner, not
a side effect of publishing the website.

While it is unset, `/shop/`, the product page and the homepage's featured strip all say
**"The catalogue is not connected yet"** and route the visitor to the free guides and articles.
They specifically do *not* say "temporarily unavailable" — that would imply a working shop
having a bad moment, which is not what is happening.

Setting `SHOP_API_BASE` to a deployed Worker URL is the single change that lights up the whole
storefront. Nothing else needs editing.
...
## 9. Needs a decision from the Owner
...
- **Point the storefront at a Worker.** See section 7. Until then the shop is honest but empty.
