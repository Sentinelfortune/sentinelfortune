...
describe("admin proxy — authenticated request succeeds", () => {
  ...
  it("maps /api/<path> onto the Worker origin and carries the token in the header", async () => {
    ...
    // Both sides go through Request so the comparison survives URL host
    // normalization — a hostname is case-insensitive and gets lower-cased on
    // parse, which matters while DEFAULT_WORKER_ORIGIN is still a
    // REPLACE_WITH_… placeholder written in capitals.
    expect(forwarded[0].url).toBe(new Request(`${DEFAULT_WORKER_ORIGIN}/shop/admin/whoami`).url);
